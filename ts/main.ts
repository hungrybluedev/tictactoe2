...
const populateButtons = (state: State) => {
  for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 3; j++) {
      const id = `${i}${j}`;
      ...
      button.addEventListener("click", () => {
        // If a button is clicked, it's 100% a human player
        click(state, 3 * i + j);

        if (state.mode === GameMode.HVC) {
          // The human must have clicked.
          // So now we allow the CPU to click.
...
const updateSummary = (state: State, index: number) => {
...
const getButton = (id: string): HTMLButtonElement | null => {
  ...
  return element ? (element as HTMLButtonElement) : null;
};

const getMode = (name: string | null): GameMode => {
  switch (name) {
    case "HVH":
      return GameMode.HVH;
    case "HVC":
...
const initializeState = (): State => {
  let state = {
    buttons: new Array<HTMLButtonElement>(),
...
const click = (state: State, index: number) => {
...
const announce = (message: string) => {
  ...
  const messageBox = element as HTMLDivElement;
  messageBox.textContent = message;
};

const announceWinner = (player: number) => {
...
const cycleMode = (name: string): string => {
  switch (name) {
    case "HVH":
      return "HVC";
    case "HVC":
